...
<head>
    ...
    <link rel="icon" type="image/png" href="<?= base_url(); ?>slot/logo.png" sizes="16x16">
...
<body style="margin-bottom: 70px;" class="bg-light">
    ...
    <div class="container">
        ...
        <div class="modal fade" id="hadiah" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-white" style="border-radius:10px;">

                    <div class="modal-body modal_body_hadiah text-white border-white text-center" style="border-radius:10px;font-size:medium;font-weight:bold;">
    ...
    <script>
        ...
        $('#btn-example2').click(function() {
            ...
            <?php endif; ?>
            <?php if (settings()['versi'] == 2 && session('id')) : ?>
                let jml_taruhan = $('.jml_taruhan').val();
                post('slotgacor/cek_saldo', {
                        jml_taruhan
                    })
                    .then(res => {
                        if (res.status == '200') {

                            post('slotgacor/angka_terbaik_bandar_v2', {
                                    id: ''
                                })
                                .then(res => {
                                    if (res.status == '200') {
                                        let data = res.data.angka_taruhan.split(",");

                                        $('#example2 ul').playSpin({
                                            endNum: [data[0], data[1], data[2]],
                                        });

                                        let html = '';
                                        if (res.data.hadiah == 0) {
                                            let kalah = '<?= user()['jml_taruhan']; ?>';
                                            html += 'MOHON MAAF ANDA NYONYOR ' + rupiah(kalah.toString(), 'RP ');
                                            $('.modal_body_hadiah').addClass('bg-danger');
                                            $('.modal_body_hadiah').removeClass('bg-success');

                                            $('.modal_body_hadiah').html(html);
                                            let myModal = document.getElementById('hadiah');
                                            let modal = bootstrap.Modal.getOrCreateInstance(myModal);
                                            post('slotgacor/update_saldo_kemenangan', {
                                                    hadiah: res.data.hadiah,
                                                    order: 'kalah'
                                                })
                                                .then(res => {
                                                    if (res.status == '200') {
                                            ...
                                            html += 'SELAMAT ANDA MENANG ' + rupiah(res.data.hadiah.toString(), 'RP ');
                                            $('.modal_body_hadiah').removeClass('bg-danger');
                                            $('.modal_body_hadiah').addClass('bg-success');
        ...
        $('.sodaqoh').blur(function() {
            let id = $(this).data('id');
            let val = $(this).text();

            post('slotgacor/update_sodaqoh', {
                    id,
                    val
                })
                .then(res => {
