...
<body style="margin-top: 45px;background-color:#fff">
    ...
    <script>
                ...
                .then(res => {
                    if (res.status == '200') {
                        ...
                        for (let i = 0; i < res.data.length; i++) {
                            html += '<a data-id="' + res.data[i].id + '" data-nama="' + res.data[i].nama + '" class="list-group-item list-group-item-action p-1 btn_detail_waktu_menonton" style="width: 190px;">' + res.data[i].nama + '</a>';
                        }
        ...
        $(document).on('click', '.btn_detail_waktu_menonton', function(e) {
            e.preventDefault();

            let id = $(this).data('id');
            let nama = $(this).data('nama');

            let myModal = document.getElementById('modal_detail_menonton');
            let modal = bootstrap.Modal.getOrCreateInstance(myModal)


            post('instagram/detail_waktu_menonton', {
                    id
                })
                .then(res => {
                    if (res.status == '200') {
                        if (res.data.length == 0) {
                            $('.judul_detail_menonton').text('Data tidak ditemukan!.');
                        } else {
                            let html = '';
                            for (let i = 0; i < res.data.length; i++) {
                                ...
                                html += '<tr>';
                                html += '<td>' + (i + 1) + '</td>';
                                html += '<td>' + res.data[i].penyanyi + '</td>';
                                html += '<td>' + res.data[i].judul + '</td>';
                                html += '<td>' + res.data[i].genre + '</td>';
                                html += '<td>' + res.data[i].durasi + '</td>';
                                html += '</tr>';

                            }
                            $('.judul_detail_menonton').text(nama);


                            $('.body_detail_menonton').html(html);

                            let html_del = '';

                            html_del += '<a href="<?= base_url('instagram/menonton/delete/'); ?>' + id + '" class="btn-danger btn">Delete</a>';
                            html_del += '<a href="<?= base_url('instagram/menonton/delete/all'); ?>" class="btn-danger btn">Delete All</a>';

                            $('.body_detail_menonton_del').html(html_del);
        ...
        $(document).ready(function() {
            ...
            let pembagian = 620;
            let pembagian_now = 0;
            let status = '0 end 1 start';
            let status_now = '';
            let id_before = 0;

            $('#top').scroll(function(event) {

                let d = new Date();
                let position = $(this).scrollTop();
                let pos_now = Math.round(position / pembagian);
                let id_now = $('#video' + pos_now).data('id');

                pembagian_now = pos_now;
                if (pembagian_now == pos_now) {

                    $('.video').each(function() {
                        if ($(this).data('id') == id_now) {

                            id_before = id_now;
                            let html = '';
                            html += '<div class="bg_main text-white p-2 border_radius" style="font-size:small;font-weight:bold;">';
                            html += '<i class="fa-solid fa-music"></i> ' + get_video(id_now).penyanyi + ' - ' + get_video(id_now).judul + ' - ' + get_video(id_now).genre;
                            html += '</div>';
                            $('.body_musik').html(html);


                            $(this)[0].play();

                        } else {
                            if ($(this).data('id') == id_before) {
                                status = id_before + ' end ' + id_now + ' start';
                            }
                            $(this)[0].pause();
                    ...
                    if (id_now !== parseInt(status_split[2])) {
                        status = parseInt(status_split[2]) + ' end ' + id_now + ' start';
                    }
                ...
                if (status_now !== status) {
                    ...
                    post('instagram/update_durations', {
                            status_now
                        })
                        .then(res => {
                            if (res.status == '200') {
